...
**Project:** govpn (`github.com/8upio/govpn`, package `ovpn`) — pure-Go OpenVPN server protocol library
**Researched:** 2026-08-22
**Confidence:** MEDIUM-HIGH (crypto/protocol claims verified directly against OpenVPN's reference C source on GitHub; ecosystem/tooling claims are standard web-search confidence — see Sources)
...
There is no RFC for the OpenVPN protocol — `github.com/OpenVPN/openvpn` (C reference implementation) *is* the spec, and the project's own constraints correctly treat it that way. Verified directly against the current `master` branch source (Aug 2026):
...
**Not needed for v1 scope** (per PROJECT.md's explicit Out of Scope): `ssl_ncp.c` (NCP/cipher negotiation), `push.c` beyond the minimal `ifconfig`/topology push options, `crypto_epoch.c` (epoch data format is a 2.6+ optional feature — verify whether the reference 2.6 client in the Docker harness defaults to epoch or classic format before assuming the 4-byte packet-id path is sufficient; if the test client negotiates epoch format by default, that widens the AEAD nonce/packet-id handling that must be implemented even for a "fixed cipher, no NCP" v1).
...
**If NCP (cipher negotiation) is added in a later milestone:**
...
**If a future `ovpn/tun` package is built (Linux first, per the briefing):**
...
---
*Stack research for: pure-Go OpenVPN server protocol library*
*Researched: 2026-08-22*
