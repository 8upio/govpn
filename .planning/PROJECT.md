...
- **There is no clean RFC for the OpenVPN protocol.** The C reference implementation (`ssl.c`, `crypto.c` in the openvpn repo) *is* the spec. Every byte-level protocol detail (opcodes, packet formats, key derivation) must be verified against that source, never guessed from training data.
