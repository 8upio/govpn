# Milestone v1.0 — Archived 2026-08-28

**Shipped:** 2026-08-22 → 2026-08-28 · 4 phases · 18 plans · 193 commits · ~28.7k LOC Go (stdlib-only core)

## Key Accomplishments
1. A real, unmodified OpenVPN 2.6.14 client completes a full TLS 1.3 mutual-cert handshake over tls-crypt against the pure-Go library — verified by packet capture, golden vectors, and a loss-injecting Docker harness
2. Key Method 2 key derivation and the AES-256-GCM data channel, byte-exact against the C reference — client tunnels up ("Initialization Sequence Completed") and encrypted IP packets round-trip through the embedder's Session
3. Hand-built userspace netstack (UDP + minimal server-side TCP + ICMP echo): stdlib http.Serve runs unmodified over the tunnel in an unprivileged container (no TUN, no CAP_NET_ADMIN), with the tunnelweb example reachable only through the tunnel
4. Durable sessions: bidirectional soft-reset renegotiation with lame-duck key transition (3 live rollovers, zero reconnects), explicit-exit-notify immediate close, idle reaping, and a 20-cycle soak with flat goroutine/heap counts
5. Quality rails throughout: 20 standing AST prohibition gates, golden vectors from real client traffic, per-phase threat models (all closed), and adversarial review/fix loops that caught 5 critical bugs before ship
